...
## 16. 재실행 시 LVM 논리 볼륨 생성 실패

### 증상
전체 재현 검증 중 db 계층 플레이북이 실패:

```
FAILED: "Sorry, no shrinking of pglv to 0 permitted."
```

최초 구축 시에는 정상 동작했던 task다.

### 원인
`community.general.lvol` 모듈에 `size: 100%FREE`를 지정했다. 이는 볼륨 그룹의 **미사용 공간 전체**를 할당하라는 의미다. 최초 실행 시에는 전체 공간이 미사용이므로 정상이지만, 이미 전체가 할당된 상태에서 재실행하면 미사용 공간이 0이 되어 "논리 볼륨을 0으로 축소하라"는 요청으로 해석된다.

최초 구축만 고려하고 재실행을 고려하지 않은 전형적인 멱등성 결함이다.

### 해결
논리 볼륨 존재 여부를 먼저 확인하고, 없을 때만 생성하도록 수정했다.

```yaml
- name: Check if logical volume exists
  ansible.builtin.stat:
    path: "/dev/{{ pg_vg_name }}/{{ pg_lv_name }}"
  register: pg_lv
...
- name: Create logical volume using all space
  community.general.lvol:
    ...
    lv: "{{ pg_lv_name }}"
    size: 100%FREE
  when: not pg_lv.stat.exists
```

### 교훈
- 상대적 크기 지정(`100%FREE`, `+10G`)은 현재 상태에 의존하므로 재실행 시 의미가 달라진다. 절대 크기를 쓰거나, 존재 여부로 가드를 거는 것이 안전하다.
- **재현 검증은 반드시 수행해야 한다.** 최초 구축 성공만으로 "재현 가능하다"고 말할 수 없다. 이 결함은 플레이북을 두 번째로 실행하기 전까지 드러나지 않았다.

---

## 17. 서비스 롤과 방화벽 롤의 설정 충돌 (드리프트)

### 증상
전체 재현 검증 중, 플레이북을 반복 실행해도 `changed`가 계속 발생했다. 확인해보니 서로 반대되는 작업이었다.

```
monitoring.yml → node-exporter : Open exporter port in firewalld → changed
firewall.yml   → firewall : Close open node_exporter port        → changed
```

### 원인
동일한 자원(9100 포트)을 두 롤이 서로 다른 목표 상태로 관리하고 있었다. 서비스 롤은 구축 편의를 위해 포트를 열고, 나중에 추가한 방화벽 롤은 보안 정책상 그 포트를 닫는다.

결과적으로 **어느 플레이북을 마지막에 실행했느냐에 따라 최종 상태가 달라지는** 불안정한 구성이 되었다. 이를 설정 드리프트라 한다.

### 해결
관심사를 분리했다. 서비스 롤에서 방화벽 관련 task를 제거하고, 접근 제어는 firewall 롤이 단독으로 관리하도록 했다.

- node-exporter, prometheus, alertmanager, nginx 롤: 포트 개방 task 제거
- haproxy 롤: stats(8404) 개방 제거, 서비스 포트(80)만 유지
- firewall 롤: 모든 접근 제어 정책을 출발지 제한과 함께 정의

수정 후 재실행 시 전 노드 `changed=0`으로 수렴했다.

### 교훈
- **하나의 자원은 하나의 주체가 관리한다.** 여러 롤이 같은 설정을 다루면 실행 순서에 따라 결과가 달라지고, 멱등성이 깨진다.
- 이런 충돌은 단일 플레이북만 실행할 때는 드러나지 않는다. 전체를 순차 실행하는 재현 검증이 필요한 이유다.
- `changed`가 매번 발생한다면 멱등성을 의심해야 한다. 정상이라면 두 번째 실행부터는 `changed=0`이어야 한다.
