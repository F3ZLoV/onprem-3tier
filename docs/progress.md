...
### 목표
포트 개방 여부만 관리하던 방화벽을 출발지 기반 접근 제어로 전환하고, 정책을 전용 롤로 일원화한다.

### 설계 판단
- **VLAN 분리 대신 접근 제어 강화**: 당초 계획은 계층별 VLAN 분리였으나, 정상 동작 중인 7노드의 네트워크 구성을 재설계해야 하고 VIP·복제·관측이 모두 영향을 받는다. 위험 대비 실익이 낮다고 판단해, 동일한 목적(계층 간 통신 최소화)을 접근 제어로 달성했다.
- **출발지 제한**: 기존에는 모든 포트가 전 대역에 개방되어 있었다. 실제로 필요한 출발지만 허용하도록 rich rule로 전환했다.
- **정책 일원화**: 방화벽 설정이 각 서비스 롤에 분산되어 있어, 서비스 롤은 포트를 열고 방화벽 롤은 닫는 충돌이 발생했다. 서비스 롤에서 방화벽 task를 제거하고 firewall 롤이 단독으로 관리하도록 관심사를 분리했다.

### 사전 점검에서 발견한 문제
| 항목 | 내용 |
|---|---|
| cockpit 서비스 | 전 노드에 기본 개방. 사용하지 않는 웹 관리 콘솔이며 Prometheus와 포트(9090)가 겹침 |
| db2의 nginx | 클론 원본(web2)에서 딸려온 웹 서버가 DB 노드에서 동작 중. 불필요한 공격 표면 |
| db1/db2 비대칭 | db1만 postgresql 서비스가 개방되어 있어, 페일오버 시 db2가 primary가 되면 접근 불가 |
...
| db | 5432 | db1, db2 (상호) |
| 전 노드 | 9100 | mon |
| lb | 8404 (stats) | mon, 관리 호스트 |
| mon | 9090, 9093 | 관리 호스트 |
| lb | 80 | 제한 없음 (서비스 진입점) |
| mon | 3000 (Grafana) | 제한 없음 (브라우저 접근) |

각 항목은 **허용 규칙 추가 → 기존 개방 제거** 순으로 배치했다. 순서가 반대면 정책 전환 중 통신이 단절되어 수집 실패나 알림 오탐이 발생한다.

SSH는 정책 대상에서 의도적으로 제외했다. 원격 관리 경로가 차단되면 Ansible로 복구할 수 없게 되어, VM 콘솔로 전 노드를 수동 복구해야 한다.

### 검증
적용 후 각 계층의 정상 동작을 확인했다.
- VIP를 통한 서비스: web1/web2 분산 정상
- 컨트롤 노드에서 web 직접 접근: **차단됨**(의도한 결과)
- DB 복제: `pg_is_in_recovery()` = t 유지
- 메트릭 수집: Prometheus 타겟 7/7 up

접근을 제한하면서도 모든 정상 경로가 유지되는 것을 확인했다. 방화벽 변경은 적용 자체보다 적용 후 검증이 중요하다.
...
### 목표
전체 플레이북을 재실행해 재현 가능성을 검증하고, 구성 문서를 최종 정리한다.

### 재현 검증에서 발견한 결함
전 플레이북을 순차 실행한 결과 두 가지 문제가 드러났다. 최초 구축은 성공했으나 **재실행 시 실패하거나 상태가 변동**하는 결함으로, 실행해보지 않았다면 발견할 수 없었다.

**1. LVM 논리 볼륨 생성 실패**
```
FAILED: "Sorry, no shrinking of pglv to 0 permitted."
```
`size: 100%FREE`는 볼륨 그룹의 미사용 공간 전체를 할당한다. 최초 실행 시에는 정상이지만, 이미 전체를 할당한 상태에서 재실행하면 "남은 공간 0"이 되어 축소 시도로 해석된다. `stat`으로 논리 볼륨 존재 여부를 확인해 이미 생성된 경우 건너뛰도록 수정했다.

**2. 방화벽 설정 드리프트**
서비스 롤(`Open exporter port`)과 방화벽 롤(`Close open node_exporter port`)이 동일 포트를 반대로 관리하고 있었다. 플레이북을 실행할 때마다 열고 닫기를 반복해, 실행 순서에 따라 최종 상태가 달라지는 상태였다. 서비스 롤에서 방화벽 task를 제거하고 firewall 롤로 일원화했다.

### 최종 검증 결과
수정 후 전 플레이북을 재실행한 결과:

| 플레이북 | 결과 |
|---|---|
| web.yml | failed=0 |
| lb.yml | failed=0 |
| db.yml | failed=0, changed=0 |
| backup.yml | failed=0, changed=0 |
| monitoring.yml | failed=0, **전 노드 changed=0** |
| firewall.yml | failed=0, **전 노드 changed=0** |

`changed=0`은 현재 상태가 코드에 정의된 상태와 완전히 일치함을 의미한다. 몇 번을 실행해도 결과가 동일하며, 이것이 선언형 구성 관리의 목표 상태다.

(`restorecon`만 매 실행 changed로 표시되는데, `command` 모듈은 Ansible이 상태를 판단할 수 없기 때문이다. 실제 변경은 발생하지 않는다.)
...
```
   ...
   ansible (.10)          mon (.50)
   플레이북 배포            Prometheus · Grafana · Loki
```

모든 접근은 firewalld rich rule로 출발지가 제한된다.
